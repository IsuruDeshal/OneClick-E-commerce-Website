 ...
 * - Accepts either wishlist_item_id (RLS filters by user) OR product_id (must match user's wishlist)
 * - Uses RLS enforcement; anon key can only delete own items
 *
 * POST /api/wishlist/remove
 * Body: { product_id: string|number } OR { wishlist_item_id: string }
 *
...
// Accept either wishlist_item_id OR product_id
$wishlist_item_id = $input['wishlist_item_id'] ?? null;
$product_id = $input['product_id'] ?? null;

if (!$wishlist_item_id && !$product_id) {
    json_validation_error('product_id|wishlist_item_id', 'Either product_id or wishlist_item_id is required');
}

try {
    // Build DELETE filter
    if ($wishlist_item_id) {
        // Delete by wishlist_item_id (RLS ensures user ownership)
        $filter = 'id=eq.' . urlencode($wishlist_item_id);
    } else {
    ...
    $path = '/rest/v1/wishlist_items?' . $filter;
    [$code, $resp] = supabase_request_anon('DELETE', $path);

    // 204 (No Content) or 200 is success
    if ($code !== 204 && $code !== 200) {
        throw new Exception("Failed to remove from wishlist (HTTP $code): $resp");
    }
